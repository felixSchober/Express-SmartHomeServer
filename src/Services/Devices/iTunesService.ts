    ...
    getIsMuteState(): Promise<boolean> {
        ...
        return new Promise<boolean>((resolve, reject) => {
            iTunesService.doiTunesGetRequest(path)
                ...
                .catch(error => {
                    console.log('[iTunes] getIsMuteState() - Could not get mute state from server: ' + JSON.stringify(error));
                    reject('[iTunes] getIsMuteState() - Could not get mute state from server: ' + JSON.stringify(error));
                });
        });
    }
    ...
    toggleMuteUnmute(): Promise<boolean> {
        ...
        return new Promise<boolean>((resolve, reject) => {
            iTunesService.doiTunesPostRequest(path, {})
                ...
                .catch(error => {
                    console.log('[iTunes] toggleMuteUnmute - Could not get state from server: ' + JSON.stringify(error));
                    reject('[iTunes] toggleMuteUnmute - Could not get state from server: ' + JSON.stringify(error));
                });
        });
    }

    skipTrack(): Promise<ITrack> {
        const path = `player/skip`;
        return new Promise<ITrack>((resolve, reject) => {
            iTunesService.doiTunesPostRequest(path, {})
                .then(response => {
                    const result = response.data as ITrack;
                ...
                .catch(error => {
                    console.log('[iTunes] skipTrack - Could not get state from server: ' + JSON.stringify(error));
                    reject('[iTunes] skipTrack - Could not get state from server: ' + JSON.stringify(error));
                });
        });
    }

    previousTrack(): Promise<ITrack> {
        const path = `player/previous`;
        return new Promise<ITrack>((resolve, reject) => {
            iTunesService.doiTunesPostRequest(path, {})
                .then(response => {
                    const result = response.data as ITrack;
                ...
                .catch(error => {
                    console.log('[iTunes] prevTrack - Could not get state from server: ' + JSON.stringify(error));
                    reject('[iTunes] prevTrack - Could not get state from server: ' + JSON.stringify(error));
                });
        });
    }
